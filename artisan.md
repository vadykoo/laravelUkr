...
Artisan - це інтерфейс командного рядка, що входить до складу Laravel. Він надає ряд корисних команд, які можуть допомогти вам під час створення вашої програми. Щоб переглянути список усіх доступних команд Artisan, ви можете використовувати`list`команда:

    php artisan list

Кожна команда також включає екран "довідки", який відображає та описує доступні аргументи та параметри команди. Щоб переглянути екран довідки, перед назвою команди натисніть`help`:
...
Laravel Tinker - це потужний REPL для фреймворку Laravel, який працює від[PsySH](https://github.com/bobthecow/psysh)пакет.
...
> {tip} Шукаєте графічний інтерфейс для взаємодії з вашим додатком Laravel? Перевіряти[Тінкервелл](https://tinkerwell.app)!

<a name="usage"></a>

#### Використання

Tinker дозволяє взаємодіяти з усім вашим додатком Laravel у командному рядку, включаючи Eloquent ORM, завдання, події тощо. Щоб увійти в середовище Tinker, запустіть`tinker`Команда ремісників:
...
> {примітка}`dispatch`допоміжна функція та`dispatch`метод на`Dispatchable`клас залежить від збору сміття, щоб розмістити завдання в черзі. Тому, використовуючи майстерність, слід використовувати`Bus::dispatch`або`Queue::push`відправляти робочі місця.
...
Як правило, Tinker автоматично псевдонімить класи, коли вони потрібні в Tinker. Тим не менш, ви можете ніколи не псевдонім деяких класів. Ви можете досягти цього, перелічивши класи в`dont_alias`масив вашого`tinker.php`файл конфігурації:
...
На додаток до команд, наданих Artisan, ви також можете створити власні власні команди. Команди зазвичай зберігаються в`app/Console/Commands`каталог; однак ви можете вибрати власне місце зберігання, доки ваші команди може завантажувати Composer.
